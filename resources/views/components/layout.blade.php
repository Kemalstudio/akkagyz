...
<head>
    ...
    <style>
        ...
        @media(max-width:720px){.header-service{display:none}.header-main{min-height:64px;padding:10px 14px;gap:8px}.header-logo img{max-width:122px!important;height:30px!important}.header-stores-btn{display:none}.header-divider{display:none}.account-name{display:none}.account-btn{padding:3px}.header-search .input{height:42px;font-size:13px}.catalog-trigger-btn{padding:0 12px!important;font-size:12px!important}.header-actions .icon-btn{width:37px;height:37px}.desktop-auth-register{display:none}.mobile-only{display:flex}}
        @media(max-width:760px){.header-actions>a[title="Избранное"],.header-actions>a[title="Корзина"]{display:none}}
        ...
        .footer-grid{display:grid;grid-template-columns:1.4fr 1fr 1fr 1fr;gap:32px;padding:48px 24px 32px}
        .footer-bottom{display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;padding:18px 24px}
        @media(max-width:780px){.footer-grid{grid-template-columns:repeat(2,1fr);padding:38px 18px 26px;gap:24px}.footer-bottom{padding:16px 18px}}
...
<footer style="border-top:1px solid var(--border);background:var(--bg-elevated);margin-top:56px;">
    <div class="wrap footer-grid">
        <div>
            <x-logo :height="32" />
            <div style="font-size:13px;color:var(--text-faint);margin-top:14px;max-width:260px;line-height:1.5;">Магазин канцтоваров, упаковки и товаров для творчества от {{ $businessSettings->site_name }}. Собственный ассортимент, быстрая доставка по Туркменистану.</div>
        </div>
    ...
    <div style="border-top:1px solid var(--border);">
        <div class="wrap footer-bottom" style="color:var(--text-faint);font-size:13px;">
            <span>&copy; {{ date('Y') }} {{ $businessSettings->site_name }}. Все права защищены.</span>
        </div>
    </div>
</footer>

<x-mobile-nav variant="main" :cart-count="$cartCount" :wishlist-count="$wishlistCount" />
