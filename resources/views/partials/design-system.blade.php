...
.cart-drawer-backdrop{position:fixed;inset:0;z-index:180;background:rgba(8,15,30,.42);backdrop-filter:blur(3px);opacity:0;visibility:hidden;transition:.28s}.cart-drawer{position:fixed;right:0;top:0;bottom:0;z-index:181;width:min(430px,100%);background:var(--surface);box-shadow:-24px 0 60px rgba(5,12,30,.2);transform:translateX(102%);transition:transform .36s cubic-bezier(.22,.8,.25,1);display:flex;flex-direction:column}.cart-drawer-backdrop.show{opacity:1;visibility:visible}.cart-drawer.show{transform:translateX(0)}.cart-drawer-head{padding:20px 22px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between}.cart-drawer-title{font-size:19px;font-weight:700;display:flex;align-items:center;gap:9px}.cart-drawer-close{width:36px;height:36px;border:1px solid var(--border);border-radius:10px;background:var(--bg);display:grid;place-items:center;cursor:pointer}.cart-drawer-body{padding:8px 20px;overflow:auto;flex:1}.cart-drawer-item{display:flex;align-items:flex-start;gap:12px;padding:14px 0;border-bottom:1px solid var(--border)}.cart-drawer-image{flex:0 0 64px;width:64px;height:64px;border-radius:12px;background:var(--bg) center/contain no-repeat;display:grid;place-items:center;color:var(--text-faint)}.cart-drawer-item-body{flex:1;min-width:0}.cart-drawer-name{display:block;font-size:13px;font-weight:650;line-height:1.35}.cart-drawer-qty{font-size:11px;color:var(--text-faint);margin-top:4px}.cart-drawer-item-bottom{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:9px}.cart-drawer-stepper{display:flex;align-items:center;height:28px;border:1px solid var(--border);border-radius:9px;overflow:hidden;background:var(--bg)}.cart-drawer-stepper button{width:26px;height:100%;border:0;background:transparent;color:var(--text);display:grid;place-items:center;cursor:pointer}.cart-drawer-stepper button:hover:not(:disabled){background:var(--accent-soft);color:var(--accent)}.cart-drawer-stepper button:disabled{opacity:.35;cursor:not-allowed}.cart-drawer-stepper span{min-width:22px;text-align:center;font-size:12.5px;font-weight:750}.cart-drawer-remove{width:28px;height:28px;flex-shrink:0;border:1px solid var(--border);border-radius:9px;background:var(--bg);color:var(--text-faint);display:grid;place-items:center;cursor:pointer;transition:.15s}.cart-drawer-remove:hover{color:var(--danger);border-color:var(--danger)}.cart-drawer-price{flex:0 0 auto;text-align:right;font-size:13px;font-weight:750;white-space:nowrap}.cart-drawer-foot{padding:18px 22px 22px;border-top:1px solid var(--border);background:var(--bg-elevated)}.cart-drawer-total{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:15px}.cart-drawer-total span{font-size:13px;color:var(--text-muted)}.cart-drawer-total strong{font-size:22px}.cart-drawer-actions{display:grid;grid-template-columns:1fr 1.3fr;gap:9px}.cart-drawer-empty{text-align:center;padding:70px 15px;color:var(--text-faint)}.cart-drawer-express{display:flex;width:100%;align-items:center;justify-content:space-between;gap:10px;height:44px;margin-top:9px;padding:0 15px;border:1px dashed color-mix(in oklch,var(--warning) 45%,var(--border));border-radius:12px;background:var(--warning-soft);color:var(--warning);font:inherit;font-weight:800;font-size:13px;cursor:pointer;transition:.15s}.cart-drawer-express:hover{border-style:solid;transform:translateY(-1px)}.cart-drawer-express-label{display:flex;align-items:center;gap:7px}.cart-drawer-express-fee{font-size:11.5px;font-weight:900;padding:3px 8px;border-radius:8px;background:var(--surface);color:var(--warning)}.cart-drawer-express:disabled{opacity:.55;cursor:not-allowed;transform:none}
.badge{font-size:11px;font-weight:800;padding:4px 8px;border-radius:8px;display:inline-flex;align-items:center;gap:4px;}
.product-ribbons{position:absolute;z-index:3;top:12px;left:0;display:flex;flex-direction:column;align-items:flex-start;gap:7px;filter:drop-shadow(0 7px 8px rgba(8,15,35,.16));pointer-events:none}
.product-ribbon{--ribbon-bg:var(--accent);position:relative;min-height:28px;padding:0 20px 0 11px;clip-path:polygon(0 0,calc(100% - 9px) 0,100% 50%,calc(100% - 9px) 100%,0 100%);background:var(--ribbon-bg);color:#fff;display:inline-flex;align-items:center;gap:5px;font-size:10.5px;font-weight:900;line-height:1;text-transform:uppercase;letter-spacing:.045em;text-shadow:0 1px 1px rgba(0,0,0,.16)}
.product-ribbon:before{content:'';position:absolute;left:0;bottom:-5px;border-top:5px solid color-mix(in srgb,var(--ribbon-bg) 66%,#000);border-left:5px solid transparent}
.product-ribbon--discount{--ribbon-bg:#d92f43}.product-ribbon--new{--ribbon-bg:#17833a}.product-ribbon--vip{--ribbon-bg:#b57808;color:#fff6d9}.product-ribbon--used{--ribbon-bg:#5b6472}
...
.rgrid-4{display:grid;grid-template-columns:repeat(4,1fr);gap:14px}
.rgrid-sidebar{display:grid;grid-template-columns:220px 1fr;gap:24px}
.rgrid-main{display:grid;grid-template-columns:1.3fr 1fr;gap:20px;align-items:start}
@media(max-width:900px){.rgrid-4{grid-template-columns:repeat(2,1fr)}}
@media(max-width:640px){.rgrid-2,.rgrid-sidebar,.rgrid-main{grid-template-columns:1fr}}
...
@media(max-width:1050px){.product-grid-4{grid-template-columns:repeat(3,1fr)}}
@media(max-width:760px){.product-grid-4{grid-template-columns:repeat(2,1fr)}}
@media(max-width:480px){.product-grid-4{grid-template-columns:1fr}}
.notif-panel{width:360px}
@media(max-width:480px){.notif-panel{position:fixed!important;left:12px;right:12px;top:64px;width:auto}}
