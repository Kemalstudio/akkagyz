...
<div class="wrap catalog-layout" style="padding:12px 24px 40px;display:grid;grid-template-columns:280px 1fr;gap:30px;">
    <aside class="catalog-sidebar">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;">
            ...
            @if(request()->anyFilled(['q','category','min_price','max_price','in_stock','brands','rating_min','on_sale','vip']))
        ...
        <form method="GET" action="{{ route($routeName) }}">
            ...
            <div>
                <div style="font-size:14px;font-weight:800;margin-bottom:12px;">Цена, TMT</div>
            ...
            <div style="border-top:1px solid var(--border);padding-top:18px;margin-top:18px;">
                <label style="display:flex;align-items:center;gap:10px;font-size:14px;color:var(--text-muted);font-weight:600;cursor:pointer;">
                    <input type="checkbox" name="in_stock" value="1" {{ request()->boolean('in_stock') ? 'checked' : '' }} style="width:18px;height:18px;accent-color:var(--accent);">
                    Только в наличии
                </label>
            </div>

            <div class="filter-section"><div class="filter-label">Покупательский рейтинг</div>@foreach([4=>'4 ★ и выше',3=>'3 ★ и выше'] as $rating=>$label)<label class="filter-option"><input type="radio" name="rating_min" value="{{ $rating }}" @checked((int)request('rating_min')===$rating)><span>{{ $label }}</span></label>@endforeach</div>
            <div class="filter-section"><label class="filter-option"><input type="checkbox" name="on_sale" value="1" @checked(request()->boolean('on_sale'))><span>Товары со скидкой</span></label>@if($routeName==='marketplace.catalog')<label class="filter-option"><input type="checkbox" name="vip" value="1" @checked(request()->boolean('vip'))><span>Только VIP-товары</span></label>@endif</div>

            @if($availableBrands->isNotEmpty())
                <div style="border-top:1px solid var(--border);padding-top:18px;margin-top:18px;">
                    <div style="font-size:14px;font-weight:800;margin-bottom:12px;">Бренд</div>
                    <div style="display:flex;flex-direction:column;gap:9px;max-height:220px;overflow-y:auto;">
                        @foreach($availableBrands as $brand)
                            <label style="display:flex;align-items:center;gap:10px;font-size:13.5px;color:var(--text-muted);font-weight:600;cursor:pointer;">
                                <input type="checkbox" name="brands[]" value="{{ $brand }}" {{ in_array($brand, request('brands', [])) ? 'checked' : '' }} style="width:17px;height:17px;accent-color:var(--accent);flex-shrink:0;">
                                <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $brand }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
            ...
            <form method="GET" action="{{ route($routeName) }}" style="display:flex;align-items:center;gap:10px;">
                @foreach(request()->except('sort') as $k => $v)
                    @foreach(Illuminate\Support\Arr::wrap($v) as $vItem)
                        <input type="hidden" name="{{ is_array($v) ? $k.'[]' : $k }}" value="{{ $vItem }}">
                    @endforeach
                @endforeach
                <?php
...
<style>.catalog-sidebar{display:flex;flex-direction:column;gap:20px;padding:18px;border-radius:18px;background:var(--surface);border:1px solid var(--border);height:max-content}.filter-context{display:flex;align-items:center;gap:11px;padding:4px 0 16px;border-bottom:1px solid var(--border)}.filter-context strong{display:block;font-size:13px;font-weight:700}.filter-context span:not(.entity-icon){display:block;font-size:10px;color:var(--text-faint);margin-top:3px}.quick-price-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:10px}.quick-price-grid button{min-height:31px;padding:5px;border:1px solid var(--border);border-radius:8px;background:var(--bg);color:var(--text-muted);font-size:10px}.quick-price-grid button:hover,.quick-price-grid button.active{border-color:var(--accent);color:var(--accent);background:var(--accent-soft)}.price-caption{font-size:9px;color:var(--text-faint);margin-top:8px}.filter-section{border-top:1px solid var(--border);padding-top:17px;margin-top:17px;display:grid;gap:9px}.filter-label{font-size:12px;font-weight:700;margin-bottom:3px}.filter-option{display:flex;align-items:center;gap:9px;font-size:12px;color:var(--text-muted);cursor:pointer}.filter-option input{width:16px;height:16px;accent-color:var(--accent)}.subcategory-box{display:grid;gap:4px;padding:14px;border-radius:13px;background:var(--bg);border:1px solid var(--border)}.subcat-link{display:flex;align-items:center;gap:7px;padding:7px 8px;border-radius:7px;color:var(--text-muted);font-size:11.5px}.subcat-link:hover,.subcat-link.active{background:var(--accent-soft);color:var(--accent)}@media(max-width:900px){.catalog-layout{grid-template-columns:1fr!important}.catalog-sidebar{position:static}.catalog-layout>div>div[style*="grid-template-columns:repeat(3"]{grid-template-columns:repeat(2,1fr)!important}}@media(max-width:520px){.catalog-layout{padding-left:14px!important;padding-right:14px!important}.catalog-layout>div>div[style*="grid-template-columns:repeat(3"]{grid-template-columns:1fr!important}}</style>
<script>function akPricePreset(btn){const form=btn.closest('form'),min=form.querySelector('[name=min_price]'),max=form.querySelector('[name=max_price]');min.value=btn.dataset.min||'';max.value=btn.dataset.max||'';form.querySelectorAll('.quick-price-grid button').forEach(x=>x.classList.remove('active'));btn.classList.add('active')}document.querySelectorAll('.sort-dropdown').forEach(function(dropdown){const trigger=dropdown.querySelector('.sort-trigger'),form=dropdown.closest('form'),input=form.querySelector('input[name=sort]');trigger.addEventListener('click',function(e){e.stopPropagation();const open=dropdown.classList.toggle('open');trigger.setAttribute('aria-expanded',open)});dropdown.querySelectorAll('.sort-option').forEach(function(option){option.addEventListener('click',function(){input.value=option.dataset.value;dropdown.classList.remove('open');form.submit()})})});document.addEventListener('click',function(){document.querySelectorAll('.sort-dropdown.open').forEach(function(el){el.classList.remove('open');el.querySelector('.sort-trigger').setAttribute('aria-expanded','false')})});</script>
